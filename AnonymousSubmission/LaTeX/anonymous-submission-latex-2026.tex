...
VCS computes global thematic alignment to capture overall semantic correspondence between reference and generated descriptions. Given input texts $T_{ref}$ and $T_{gen}$, we extract global embeddings using NV-Embed-v2:

\begin{equation} \label{eq:gas_embeddings}
\mathbf{E}_{ref} = \text{NV-Embed-v2}(T_{ref}), \quad \mathbf{E}_{gen} = \text{NV-Embed-v2}(T_{gen})
\end{equation}

where $\mathbf{E}_{ref}, \mathbf{E}_{gen} \in \mathbb{R}^{D}$. The GAS computes cosine similarity between these global vectors:
...
To enable fine-grained semantic comparison, VCS preprocesses input texts into structured chunk representations. Given texts $T_{ref}$ and $T_{gen}$, the preprocessing pipeline applies punctuation removal followed by Segment Any Text (SaT) segmentation to produce semantic segments:

\begin{equation} \label{eq:segmentation}
S_{ref} = \text{SaT}(\text{Clean}(T_{ref})), \quad S_{gen} = \text{SaT}(\text{Clean}(T_{gen}))
\end{equation}

To balance granularity with contextual coherence, we group $k$ consecutive segments into chunks $C_{ref} = \{c_1^{ref}, ..., c_{N_{ref}}^{ref}\}$ and $C_{gen} = \{c_1^{gen}, ..., c_{N_{gen}}^{gen}\}$. These chunks are embedded using NV-Embed-v2 to yield embedding matrices:

\begin{equation} \label{eq:chunk_embeddings}
\mathbf{E}_{C_{ref}} \in \mathbb{R}^{N_{ref} \times D}, \quad \mathbf{E}_{C_{gen}} \in \mathbb{R}^{N_{gen} \times D}
\end{equation}

This structured representation enables quantitative similarity measurement for subsequent Local and Narrative Alignment Score computation.
