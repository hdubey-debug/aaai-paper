...
When comparing chunk embeddings from $\mathbf{E}_{C_{ref}}$ and $\mathbf{E}_{C_{gen}}$, naive highest-similarity pairing creates semantic ambiguity and collisions. Chunks often exhibit high similarity with multiple candidates—for instance, both generated chunks might score 0.80 with the same reference chunk (collision), or a generated chunk might superficially match due to lexical overlap despite another reference chunk being its true narrative counterpart (ambiguity). The Best Matching Algorithm resolves these issues using adaptive context windows and Mapping Windows ($MW_{prec}$, $MW_{rec}$) to find optimal one-to-one alignments that respect narrative structure.

Given a similarity vector and mapping window constraints, the algorithm identifies the maximum similarity value and determines whether to apply context filtering based on context cutoff threshold 0.6. When applied, the context window size depends on the similarity range and control parameter 5.0. All candidates within the context threshold form the candidate set. Among these candidates, the algorithm selects the element with minimum distance to mapping window boundaries. For candidates outside windows, distance equals the minimum of left and right boundary distances; for candidates inside, distance is zero. When multiple candidates share minimum distance, the algorithm breaks ties by selecting the candidate with highest similarity value. This bidirectional process operates on the similarity matrix between $C_{ref}$ and $C_{gen}$ chunks, yielding optimal alignments balancing semantic quality with chronological constraints.
...
The Corruption Detection Test Set evaluates VCS ability to distinguish valid narrative variations from invalid content corruptions. For each of the 1,390 base descriptions, we generate 10 valid variations and 10 invalid corruptions, creating a comprehensive evaluation framework with 27,800 total test instances.

\paragraph{Valid Variations}
We create 10 types of valid variations that preserve narrative integrity while introducing legitimate stylistic and structural changes:

\textbf{Lexical Variation}: Replace words with exact synonyms while preserving sentence structure, punctuation, capitalization, and meaning. Stop words remain unchanged.

\textbf{Voice Transformation}: Convert active voice to passive voice and vice versa, maintaining meaning, structure, punctuation, and proper names without altering factual content.

\textbf{Paraphrasing}: Restructure clauses and employ synonyms while preserving exact sentence count and all factual details. No sentences are merged, split, added, or removed.

\textbf{Low-Abstraction}: Reduce word count to 50\% through sentence shortening and merging while preserving all information content and narrative sequence.

\textbf{High-Abstraction}: Reduce word count to 70\% by condensing and merging sentences while maintaining sequence and meaning without omitting details.

\textbf{Low-Elaboration}: Increase word count to 130\% through rephrasing and descriptive language addition without introducing new factual details.

\textbf{High-Elaboration}: Increase word count to 150\% using enhanced descriptive language while avoiding addition of new factual information.

\textbf{Action Decomposition}: Break actions into detailed micro-steps within continuous paragraph format, preserving story, meaning, and sequence without altering events.

\textbf{Action Aggregation}: Merge micro-actions into higher-level actions while preserving meaning, sequence, and details without information modification.

\textbf{Attribute Injection}: Add external knowledge and contextual information using real-world or plausible synthetic details, integrating naturally without removing or altering existing content.

\paragraph{Invalid Corruptions}
We generate 10 types of invalid corruptions that introduce content errors while maintaining surface plausibility:

\textbf{SAO Distortion}: Replace all subjects, actions, and objects with incorrect but plausible alternatives while maintaining narrative sequence and flow. No original subjects, actions, or objects remain.

\textbf{ID Summarization}: Reduce to 50\% word count by identifying core plot elements, characters, and conflicts, but remove crucial narrative details creating confusing gaps in comprehension.

\textbf{Minor Hallucination}: Use SAT segmentation to insert unrelated segments comprising 50\% of ground-truth segments at middle indices with proper punctuation.

\textbf{Major Hallucination}: Insert unrelated segments comprising 80\% of ground-truth segments using the same SAT-based methodology as minor hallucination.

\textbf{Minor Omission}: Delete 50\% of segments from middle positions outward using SAT segmentation while ensuring minimum two segments remain and adding proper punctuation.

\textbf{Major Omission}: Delete 80\% of segments using identical methodology as minor omission but with more aggressive content removal.

\textbf{Sequence Inversion}: Reverse complete segment order using SAT segmentation without modifying individual segment content.

\textbf{Local Permutation}: Swap adjacent segment pairs following the pattern [1,2,3,4,5] → [2,1,4,3,5] without modifying segment content or performing non-adjacent swaps.

\textbf{Sequence Rotation}: Move 50\% of segments from start to end while maintaining segment order within the moved chunk, avoiding scrambling or end-to-start rotation.

\textbf{Global Permutation}: Use SAT segmentation to randomly shuffle all segment order without modifying segment content, avoiding preservation of original sequence or deterministic reordering.

All corruption methods employ SAT model semantic segmentation via TextProcessor.sat\_segmenter(), preserve individual segment content integrity, and use configurable random seeds for reproducibility. The corruptions target structural and semantic violations while maintaining syntactic plausibility, creating challenging test cases for evaluation metrics.

\subsection{Cross-Author Consistency Test Set}

The Cross-Author Consistency Test Set evaluates VCS robustness across different authorial styles while maintaining identical semantic content. We construct this dataset using the same 1,390 scene groups from MPII-MD, generating multiple authorial versions of each base description to assess metric stability across stylistic variations.

Using the ChatGPT-4o generated descriptions as Author 1 baseline, we prompt three additional Large Language Models to transform the original chronological scene lists into coherent descriptions using distinct authorial approaches. Each model receives identical scene sequences and applies systematic transformations while preserving narrative integrity.

\paragraph{Authorial Transformation Protocol}
We instruct each LLM to transform chronological scene lists into coherent movie descriptions through eight specific stylistic modifications: paraphrasing with varied vocabulary and sentence structure, active-passive voice switching, brevity adjustments maintaining all factual content, verbosity enhancements without introducing new events, micro-macro action scaling, attribute adjustment through adjective and adverb modifications, variable detail expression emphasizing key scenes, and scene expansion with emotional or background elaboration excluding new plot points. Models maintain chronological order and preserve all events, actions, subjects, and objects while avoiding content omission, chronological alteration, new plot creation, or major content removal during attribute adjustments.

\paragraph{Model Specifications}
We employ three distinct LLMs to generate varied authorial styles:
\textbf{Author 2 (Grok)}: Grok 3 model for dynamic narrative construction.
\textbf{Author 3 (Claude)}: Claude Sonnet 3.5 for nuanced descriptive styling.
\textbf{Author 4 (Mistral)}: Mistral-Large for structured narrative presentation.

Each model processes the 1,390 scene groups independently, producing semantically equivalent descriptions with distinct authorial characteristics. This generates 5,560 cross-author descriptions (1,390 × 4 authors) enabling comprehensive evaluation of metric consistency across authorial variations while maintaining identical underlying content.
