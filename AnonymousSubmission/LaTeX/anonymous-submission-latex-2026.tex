...
Having defined Mapping Windows, VCS now establishes chunk correspondences required for LAS and NAS computation. Given the chunk embedding matrices $\mathbf{E}_{C_{ref}} \in \mathbb{R}^{N_{ref} \times D}$ and $\mathbf{E}_{C_{gen}} \in \mathbb{R}^{N_{gen} \times D}$ from preprocessing, we first compute a similarity matrix $\mathbf{S} \in \mathbb{R}^{N_{ref} \times N_{gen}}$ where $S_{i,j}$ represents the cosine similarity between the $i$-th reference chunk and $j$-th generated chunk. For each generated chunk, we need to find its best matching reference chunk, and vice versa for each reference chunk.

The naive approach would select the highest similarity value for each chunk. However, this creates semantic ambiguity and collisions. Collisions occur when two or more generated chunks yield exact same similarity values (e.g., 0.8) with the same reference chunk. Ambiguity arises when chunks have very similar similarity values (e.g., 0.88 vs 0.92), typically due to repeating events where identical descriptions appear at different temporal positions. For instance, if the same event occurs at the beginning and end of a video, naive matching might incorrectly pair the first occurrence with the last occurrence rather than maintaining temporal order.

The Best Matching Algorithm resolves these issues using adaptive context windows and Mapping Windows ($MW_{prec}$, $MW_{rec}$) to constrain the search space along the diagonal, where first chunks ideally match with first chunks and last chunks with last chunks. For each chunk, the algorithm first identifies the maximum similarity value and determines whether to apply context filtering based on context cutoff threshold 0.6. When applied, the context window size becomes $w_{ctx} = \frac{(1 - 0.6) - (1 - s_{max})}{s_{max} \cdot 5.0}$, establishing a similarity threshold that creates the candidate set of viable matches. When collisions occur within this candidate set, the algorithm computes the distance from each candidate to the mapping window boundaries and selects the candidate with minimum distance, thereby preserving chronological alignment. For candidates inside the mapping window, distance is zero; for candidates outside, distance equals the minimum separation from window boundaries. When multiple candidates share minimum distance, ties break by selecting the candidate with highest similarity value. This bidirectional process—finding best matches from generated-to-reference and reference-to-generated—yields precision-oriented ($M_P$) and recall-oriented ($M_R$) matches essential for subsequent LAS and NAS computation.
