...
Having obtained the correspondence sets $M_P$ and $M_R$ from the Best-Matching Algorithm, VCS computes the Local Alignment Score to assess fine-grained semantic alignment between matched chunk pairs. LAS averages the cosine similarities of all matched pairs in both directions. The precision component $LAS_P$ evaluates how well generated text's local semantics align with reference text, while the recall component $LAS_R$ evaluates how well reference text's local semantics align with generated text. LAS is the harmonic mean of both: $LAS = \frac{2 \cdot LAS_P \cdot LAS_R}{LAS_P + LAS_R}$. High LAS values indicate strong local semantic agreement, though this metric remains insensitive to chronological ordering, necessitating narrative assessment through NAS computation.
