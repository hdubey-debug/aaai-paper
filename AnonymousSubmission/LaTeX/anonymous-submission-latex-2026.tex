...
For chunks falling outside this tolerance zone, distance penalties are calculated based on their offset from the original Mapping Window boundary. Consider a generated chunk $c_j^{gen}$ matched to reference chunk $c_i^{ref}$, where $c_j^{gen}$ has Mapping Window $MW_{prec}[j] = [s_j, e_j)$. The raw offset is $d_{ij} = s_j - i$ when $i < s_j$ or $d_{ij} = i - (e_j - 1)$ when $i \geq e_j$. The LCT-adjusted per-chunk penalty becomes $p_{ij} = 0$ if $d_{ij} \leq \tau_{LCT} h_{LCT}$ (within tolerance), otherwise $p_{ij} = d_{ij}/N_{gen}$. Summing over all matched pairs yields $P_{total}$; the worst-case penalty is $P_{max} = N_{gen}^{-1} \sum_j \max(s_j, N_{gen} - e_j)$. The orientation score is $NAS_{D,prec} = 1 - P_{total}/P_{max}$, with $NAS_{D,rec}$ computed analogously using $M_R$ and $MW_{rec}$. The two orientations combine through harmonic mean:
