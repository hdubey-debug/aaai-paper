...
For each chunk in the generated sequence, the algorithm examines the similarity vector $\mathbf{s} = [s_1, s_2, \ldots, s_{N_{ref}}]$ containing cosine similarities with all reference chunks. The algorithm identifies $s_{max} = \max(\mathbf{s})$ and determines context window application through context range $r_{ctx} = 1 - \theta_{cutoff}$ where $\theta_{cutoff} = 0.6$. Context windowing applies when $s_{max} > 0$ and $(r_{ctx} - (1 - s_{max})) > 0$. The context window size becomes $w_{ctx} = \frac{r_{ctx} - (1 - s_{max})}{s_{max} \cdot \gamma_{ctrl}}$ with control parameter $\gamma_{ctrl} = 5.0$, establishing similarity threshold $\theta_{ctx} = s_{max} - w_{ctx}$. All elements satisfying $s_i \geq \theta_{ctx}$ form the candidate set $\mathcal{C}$.

Within $\mathcal{C}$, the algorithm computes distance $d_i$ to mapping window boundaries $[w_{start}, w_{end})$ for each candidate $i$: $d_i = 0$ if $i \in [w_{start}, w_{end})$, otherwise $d_i = \max(w_{start} - i, i - (w_{end} - 1), 0)$. The selection criterion prioritizes minimum distance: $i^* = \arg\min_{i \in \mathcal{C}} d_i$. When multiple candidates share minimum distance $d_{min}$, ties break by maximum similarity: $i^* = \arg\max_{i \in \mathcal{C}_{d_{min}}} s_i$ where $\mathcal{C}_{d_{min}} = \{i \in \mathcal{C} : d_i = d_{min}\}$. This bidirectional process operates on both precision and recall directions using the similarity matrix between $\mathbf{E}_{C_{ref}}$ and $\mathbf{E}_{C_{gen}}$, yielding optimal alignments balancing semantic quality with chronological constraints.
