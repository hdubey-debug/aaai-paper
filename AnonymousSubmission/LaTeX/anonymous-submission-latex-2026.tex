...
Multimodal embedding metrics like EMScore \citep{syxl:22} and PAC-S \citep{sbc:23} employ vision-language models such as CLIP \citep{Radford2021LearningTV} to evaluate semantic alignment between visuals and generated captions, addressing expressive variability through direct image-caption comparison, thus bypassing reference captions. EMScore computes video-caption similarity through dual-level matching: coarse-grained alignment between overall video and caption embeddings, and fine-grained matching between individual frames and words, calculating precision, recall, and F1-scores via cosine similarity for reference-free evaluation. PAC-S fine-tunes CLIP through positive-augmented contrastive learning, generating synthetic image-text pairs as additional positive samples to improve the contrastive visual-semantic space, then computes cosine similarity scores between image and caption embeddings from the enhanced model. Despite their effectiveness in short-form descriptions and strong correlation with human judgments, these metrics face computational challenges and methodological limitations when scaling to long-form descriptions.
