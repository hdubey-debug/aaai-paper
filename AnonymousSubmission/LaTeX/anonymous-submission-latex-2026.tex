...
where $P_{total,x}$ represents accumulated positional penalties and $P_{max,x}$ denotes maximum possible penalties for precision ($x=P$) or recall ($x=R$) orientations. When no violations occur, $NAS_{D,x} = 1.0$.
...
where $L_{actual}$ represents actual path length, $L_{ideal,min}$ and $L_{ideal,max}$ define ideal narrative band boundaries for precision ($x=P$) or recall ($x=R$) orientations.

The final NAS integrates both distance-based and line-based approaches through harmonic means:

\begin{align}
NAS_D &= \frac{2 \cdot NAS_{D,P} \cdot NAS_{D,R}}{NAS_{D,P} + NAS_{D,R}} \label{eq:nas_d_final} \\
NAS_L &= \frac{2 \cdot NAS_{L,P} \cdot NAS_{L,R}}{NAS_{L,P} + NAS_{L,R}} \label{eq:nas_l_final} \\
NAS_{F1} &= \frac{2 \cdot NAS_D \cdot NAS_L}{NAS_D + NAS_L} \label{eq:nas_f1_final}
\end{align}
