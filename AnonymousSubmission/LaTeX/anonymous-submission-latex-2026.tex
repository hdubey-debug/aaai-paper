...
Having defined Mapping Windows, VCS establishes chunk correspondences for LAS and NAS computation. Given chunk embedding matrices
  $\mathbf{E}_{C_{ref}} \in \mathbb{R}^{N_{ref} \times D}$ and $\mathbf{E}_{C_{gen}} \in \mathbb{R}^{N_{gen} \times D}$, we compute
  similarity matrix $\mathbf{S} \in \mathbb{R}^{N_{ref} \times N_{gen}}$ where $S_{i,j}$ represents cosine similarity between the $i$-th 
  reference and $j$-th generated chunk. We need to find the best reference match for each generated chunk, and vice versa.

  Selecting the highest similarity for each chunk creates problems. Semantic collision occurs when multiple generated chunks compete for the
   same reference chunk. Semantic ambiguity arises when a generated chunk matches a reference chunk due to word overlap, but a different 
  reference chunk represents its true narrative counterpart. For example, if Gen 10 scores 0.80 with Ref 1 and 0.79 with Ref 9, naive 
  selection chooses Ref 1, but Ref 9 may be the correct temporal match.

  The Best Matching Algorithm resolves these issues through adaptive context filtering and mapping window constraints. When maximum 
  similarity $s_{max}$ exceeds 0.6, indicating genuine semantic relatedness, we apply context window $w_{ctx} = ((1 - 0.6) - (1 - s_{max}))
  / (s_{max} \cdot 4.0)$ to consider similar-scoring candidates. This adaptive approach makes sense only when chunks discuss related 
  events—stronger similarities receive broader context windows because minor score differences often reflect positional variations of the 
  same event rather than semantic distinctions. Below 0.6, chunks likely discuss unrelated content, making context consideration 
  meaningless.

  Among candidates, we select the chunk with minimum distance to mapping window boundaries, preserving chronological alignment. This yields 
  precision matches $M_P$ and recall matches $M_R$ for LAS and NAS computation.
