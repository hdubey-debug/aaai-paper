...
\begin{abstract}
Existing video captioning evaluation metrics fail to capture the long-range chronology and semantic equivalence essential for long-form descriptions. An effective evaluation metric for long-form descriptions must (i) assess global narrative coherence, (ii) measure local semantic accuracy, and (iii) evaluate chronological consistency while detecting corrupted content. We introduce Video Comprehension Score (VCS), which directly addresses these evaluation requirements through three components: Global Alignment Score for thematic coherence, Local Alignment Score for fine-grained semantic correspondence, and Narrative Alignment Score for chronological consistency with adjustable tolerance. We evaluate VCS on two large-scale synthetic datasets designed to test corruption detection and cross-author consistency. VCS consistently outperforms traditional metrics on corruption detection tasks, being the only metric capable of distinguishing valid stylistic variations from corrupted content. On cross-author consistency evaluation, VCS achieves $>$80\% similarity scores across all authorial references when each author serves as reference in turn. VCS$_{\text{short}}$, our implementation for short captions, attains state-of-the-art human correlation on VATEX-EVAL in the 9-ref setting (Kendall's $\tau = 41.5$, Spearman's $\rho = 52.8$) and competitive results in the 1-ref setting (Kendall's $\tau = 30.0$, Spearman's $\rho = 38.1$). These results demonstrate VCS effectiveness for robust evaluation of both long-form and short-form video descriptions.
\end{abstract}
