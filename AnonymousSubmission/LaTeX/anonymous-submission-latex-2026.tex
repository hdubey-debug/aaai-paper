...
Before assessing narrative alignment, VCS defines a user-controlled parameter called Local Chronology Tolerance (LCT). This concept stems from the idea that a video can have multiple valid narratives where adjacent or concurrent events are described in different orders, and all orders are valid. To allow for this narrative variability, VCS defines LCT, where higher LCT values provide greater tolerance for re-ordering. In scenarios where $N_{ref}$ and $N_{gen}$ are divisible (either $N_{ref}$ is divisible by $N_{gen}$ or vice versa), LCT of 1 means we can swap immediate neighbors: in sequence $s_1s_2s_3s_4$, we can swap to $s_1s_3s_2s_4$ without penalty. LCT of 2 means we can swap 2nd neighbors: $s_1s_2s_3s_4$ becomes $s_3s_2s_1s_4$. In non-divisible scenarios, this exact swapping is not always possible, but LCT still adds tolerance for re-ordering—higher LCT allows farther chunks to be swapped.
