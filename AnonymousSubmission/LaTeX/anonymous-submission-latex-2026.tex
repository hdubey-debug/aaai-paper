...
VCS computes global thematic alignment to capture overall semantic correspondence between reference and generated descriptions. Given input texts $T_{ref}$ and $T_{gen}$, we extract global embeddings $\mathbf{E}_{ref}, \mathbf{E}_{gen} \in \mathbb{R}^{D}$ using NV-Embed-v2. The GAS computes cosine similarity between these global vectors:
...
To enable fine-grained semantic comparison, VCS preprocesses input texts into structured chunk representations. Given texts $T_{ref}$ and $T_{gen}$, the preprocessing pipeline applies punctuation removal followed by Segment Any Text (SaT)~\citep{frohmann-etal-2024-segment} segmentation to produce semantic segments $S_{ref}$ and $S_{gen}$. We group $k$ consecutive segments into chunks $C_{ref} = \{c_1^{ref}, ..., c_{N_{ref}}^{ref}\}$ and $C_{gen} = \{c_1^{gen}, ..., c_{N_{gen}}^{gen}\}$, which are embedded using NV-Embed-v2 to yield embedding matrices $\mathbf{E}_{C_{ref}} \in \mathbb{R}^{N_{ref} \times D}$ and $\mathbf{E}_{C_{gen}} \in \mathbb{R}^{N_{gen} \times D}$.
...
When comparing chunk embeddings $\mathbf{E}_{C_{ref}}$ and $\mathbf{E}_{C_{gen}}$ from the preprocessing stage, naive highest-similarity pairing creates semantic ambiguity and collisions. Chunks often exhibit high similarity with multiple candidates—for instance, both generated chunks might score 0.80 with the same reference chunk (collision), or a generated chunk might superficially match due to lexical overlap despite another reference chunk being its true narrative counterpart (ambiguity). The Best Matching Algorithm resolves these issues by using adaptive context windows and the Mapping Windows ($MW_{prec}$, $MW_{rec}$) defined above to find optimal one-to-one alignments that respect narrative structure.

Given a similarity vector and mapping window constraints from $MW_{prec}$ or $MW_{rec}$, the algorithm first identifies the maximum similarity value and determines whether to apply context filtering based on the context cutoff threshold (`context_cutoff_value = 0.6`). When the maximum similarity exceeds this threshold, a context window is computed as `context_window = ((context_range - (1 - max_val)) / max_val) / context_window_ctrl`, where `context_range = 1 - context_cutoff_value` and `context_window_ctrl = 5.0`. All candidates within the context threshold (`max_val - context_window`) form the candidate set. Among these candidates, the algorithm selects the element with minimum distance to the mapping window boundaries, computed as `distance = max(0, start - index, index - (end - 1))` for candidates outside the window and 0 for candidates inside. When multiple candidates share the minimum distance, the algorithm breaks ties by selecting the candidate with highest similarity value. This bidirectional process operates on the similarity matrix between $C_{ref}$ and $C_{gen}$ chunks, using columns for precision-oriented matching and rows for recall-oriented matching, yielding optimal one-to-one alignments that balance semantic quality with chronological constraints.
