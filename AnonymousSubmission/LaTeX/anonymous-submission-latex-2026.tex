...
Figure~\ref{fig:vcs-architecture} illustrates the VCS pipeline, a comprehensive metric designed to evaluate long-form video descriptions through semantic and narrative alignment assessment.
...
Mapping Windows formalize this observation by defining permissible regions where text chunks can match if stories align in narrative structure. Based on element counts $N_{ref}, N_{gen}$, the system calculates Precision Windows ($MW_{prec}$) and Recall Windows ($MW_{rec}$) that accommodate natural variations in description length and detail. The mapping windows are computed as follows:

\begin{align}
\text{slope} &= \frac{N_{longer}}{N_{shorter}} \text{ where } N_{longer} = \max(N_{ref}, N_{gen}), N_{shorter} = \min(N_{ref}, N_{gen}) \\
h_{mw} &= \lceil \text{slope} \rceil \\
\text{starts}_i &= \max(\lfloor i \times \text{slope} \rfloor, 0) \\
\text{ends}_i &= \min(\text{starts}_i + h_{mw}, N_{longer})
\end{align}

for each index $i \in [0, N_{shorter})$, creating direct windows. Reverse windows are computed by inverting the mapping relationship.
