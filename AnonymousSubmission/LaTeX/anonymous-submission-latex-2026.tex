...
For chunks falling outside this tolerance zone, distance penalties are calculated based on their offset from the original Mapping Window boundary. Consider an evaluation chunk matched to its opposite counterpart, where the evaluation chunk has a corresponding Mapping Window. If the matched chunk appears above its expected window, the raw offset measures how many rows it sits above the window start. If the matched chunk appears below its expected window, the raw offset measures how many rows it sits below the window end. The LCT-adjusted penalty becomes zero if this raw offset falls within the tolerance threshold, otherwise the penalty equals the raw offset normalized by the evaluation timeline length.

Let $P_{total}$ be the sum of all penalties across matched pairs, and $P_{max}$ be the worst-case penalty when all chunks drift to their furthest possible positions. The orientation score is computed as:
