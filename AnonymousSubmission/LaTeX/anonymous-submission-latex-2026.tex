...
For chunks falling outside this tolerance zone, distance penalties are calculated based on their offset from the original Mapping Window boundary. Consider an evaluation chunk $c_j^{eval}$ matched to opposite chunk $c_i^{opp}$, where $c_j^{eval}$ has Mapping Window $MW_{eval}[j] = [s_j, e_j)$. The raw offset is $d_{ij} = s_j - i$ when $i < s_j$ or $d_{ij} = i - (e_j - 1)$ when $i \geq e_j$. The LCT-adjusted per-chunk penalty becomes $p_{ij} = 0$ if $d_{ij} \leq \tau_{LCT} h_{LCT}$ (within tolerance), otherwise $p_{ij} = d_{ij}/N_{eval}$. The orientation score is computed as:

\begin{equation}
NAS_{D,*} = 1 - \frac{\sum_{(j,i) \in M_{eval}} p_{ij}}{N_{eval}^{-1} \sum_j \max(s_j, N_{eval} - e_j)}
\end{equation}

where the numerator sums penalties over all matched pairs and the denominator represents the worst-case penalty when all chunks drift to furthest positions. For precision, $eval = gen$, $opp = ref$, $M_{eval} = M_P$, and $MW_{eval} = MW_{prec}$; for recall, $eval = ref$, $opp = gen$, $M_{eval} = M_R$, and $MW_{eval} = MW_{rec}$. The final score combines both orientations:
