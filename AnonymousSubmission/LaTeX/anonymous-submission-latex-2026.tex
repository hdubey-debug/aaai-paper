...
Before establishing chunk correspondences, VCS defines Mapping Windows that constrain the search space for optimal alignments. This concept stems from empirical observations in text similarity matrices: when computing pairwise embeddings between identical stories, the resulting similarity matrix exhibits a clear diagonal structure that stretches or compresses proportionally with length differences.

Mapping Windows formalize this observation by defining permissible regions where text chunks can match if stories align in narrative structure. The system computes a length ratio between texts and creates windows that accommodate natural variations in description length and detail.

\begin{equation}
r = \frac{N_{longer}}{N_{shorter}}, \quad h_{mw} = \lceil r \rceil
\end{equation}

where $N_{longer} = \max(N_{ref}, N_{gen})$ and $N_{shorter} = \min(N_{ref}, N_{gen})$. For each chunk in the shorter text, a corresponding window of height $h_{mw}$ is calculated in the longer text, creating Precision Windows ($MW_{prec}$) and Recall Windows ($MW_{rec}$) through bidirectional mapping.
