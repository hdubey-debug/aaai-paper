...
However, only measuring semantic similarity may lose chronological alignment constraints, which inspires us to design distance-based penalties that enforce temporal consistency within Mapping Windows. Given the correspondence sets $M_P$ and $M_R$ from the Best-Matching Algorithm, VCS computes distance-based penalties for matches that deviate from their expected narrative positions.

The Local Chronology Tolerance defines an auxiliary window height $h_{LCT} = \lceil y/x \rceil$ that drops by one when the length ratio's fractional part lies in $(0, 0.5]$ for cases where $y > x$. A user-controlled multiplier $\tau_{LCT} \geq 0$ expands each Mapping Window by $\tau_{LCT} \cdot h_{LCT}$ rows on both sides, treating any match within this tolerance zone as chronologically acceptable with zero penalty.

For each match at index $k$ with original Mapping Window $[s, e)$, VCS computes the raw distance as $d(k) = s - k$ when $k < s$ or $d(k) = k - (e-1)$ when $k \geq e$. After applying LCT tolerance, the effective penalty becomes $p(k) = \max(0, d(k) - \tau_{LCT} \cdot h_{LCT}) / y$, with matches already inside windows incurring no penalty. The total penalty accumulates across all matches: $P_{total} = \sum_{j=1}^\ell p(k_j)$, while maximum possible penalty assumes all matches drift to the furthest timeline edges: $P_{max} = \frac{1}{y} \sum_{j=1}^\ell \max(s_j, y - e_j)$.

By such penalty normalization, it is easy to distinguish chronologically consistent matches from severe temporal violations. The orientation-specific scores become $NAS_{D,prec} = 1 - P_{total,prec} / P_{max,prec}$ and $NAS_{D,rec} = 1 - P_{total,rec} / P_{max,rec}$, which combine through harmonic mean: $NAS_D = \frac{2 \cdot NAS_{D,prec} \cdot NAS_{D,rec}}{NAS_{D,prec} + NAS_{D,rec}}$. Perfect chronological alignment yields $NAS_D = 1.0$, while severe temporal inversions approach zero. Note that length-normalized penalties modulated by $\tau_{LCT}$ cleanly separate systematic temporal violations from benign local reorderings while remaining scale-invariant across narratives of different lengths.
