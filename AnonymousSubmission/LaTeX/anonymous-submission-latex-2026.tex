...
Recent advancements in Large Video Language Models (LVLMs) \citep{Yuan2025Tarsier2,Shen2025LongVU,Ataallah2024Goldfish,Chen2025LongVILA} have enhanced automated video comprehension, enabling generation of long-form descriptions from long videos. However, evaluation reveals that LVLMs frequently (i) miss global narrative structure, (ii) fail to capture local events, and (iii) fail to establish temporal connections while often hallucinating content. These gaps indicate that LVLMs lack video comprehension capabilities, raising a question: how can we evaluate models' video comprehension? Current benchmarks utilize question-answering formats \citep{wu2024longvideobench,ataallah2024infinibench,nagrani2025neptune} or short-form description comparisons for short videos \citep{wyzs:24,chen:acl11,ZhXuCoAAAI18}, both inadequate for assessing video understanding. To evaluate video comprehension, we must move beyond question-answering and short videos/short-form descriptions, requiring models to produce long-form descriptions for comparison against video content or human-written references.

Traditional caption-based evaluation techniques include n-gram metrics such as BLEU \citep{p:02}, METEOR \citep{bl:05}, CIDEr \citep{v:15}, and ROUGE \citep{l:04}, embedding-based metrics such as BERTScore \citep{z:20}, and contemporary LLM-based approaches such as CLAIR \citep{chan:23} and AutoDQ \citep{wyzs:24}, each exhibiting critical shortcomings with extensive LVLM-generated long-form descriptions. N-gram metrics rely on lexical overlap, unfairly penalizing legitimate stylistic variations, increasingly problematic as description length grows. Embedding-based metrics address lexical overlap by focusing on semantic similarity but remain constrained by limited context lengths, less effective for long-form descriptions, and inadequately evaluate chronological alignment. LLM-driven methods provide deeper semantic insights but depend heavily on the inherent accuracy of the underlying LLM and often lack consistency.
