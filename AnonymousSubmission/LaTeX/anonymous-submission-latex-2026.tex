...
For chunks falling outside this tolerance zone, distance penalties are calculated based on their offset from the original Mapping Window boundary. For a match at row $k$ with window $[s, e)$, the raw offset is $d(k) = s - k$ when $k < s$ or $k - (e-1)$ when $k \geq e$. The LCT-adjusted per-chunk penalty becomes $p(k) = \max\{0, d(k) - \tau_{LCT} h_{LCT}\}/N_{eval}$. Summing over all windows yields $P_{total}$; the worst-case penalty is $P_{max} = N_{eval}^{-1} \sum_j \max(s_j, N_{eval} - e_j)$. The orientation score is $NAS_{D,*} = 1 - P_{total}/P_{max}$, and the two orientations combine through harmonic mean:
